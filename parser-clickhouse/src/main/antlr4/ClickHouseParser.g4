...
alterTableClause
    : ADD COLUMN (IF NOT EXISTS)? tableColumnDfnt (addAfterColumnName)?           # AlterTableClauseAddColumn
    ...
    | MODIFY COLUMN (IF EXISTS)? nestedIdentifier columnCommentClause              # AlterTableClauseModifyComment
...
addAfterColumnName: AFTER nestedIdentifier;
...
createStmt
    ...
    | (ATTACH | CREATE (OR REPLACE)? | REPLACE) TEMPORARY? TABLE (IF NOT EXISTS)? tableIdentifier uuidClause? clusterClause? tableSchemaClause? engineClause? subqueryClause? tableCommentClause?                               # CreateTableStmt
...
dictionaryAttrDfnt
locals [Dictionary<String> attrs=new Dictionary<>()]:
...
locals [Dictionary<String> clauses=new Dictionary<>()]:
...
locals [Dictionary<String> clauses=new Dictionary<>()]:
    ...
    ;
tableCommentClause: COMMENT STRING_LITERAL;
columnCommentClause: COMMENT STRING_LITERAL;
...
tableElementExpr
    ...
    ;
tableColumnDfnt
    : nestedIdentifier columnTypeExpr tableColumnPropertyExpr? columnCommentClause? codecExpr? (TTL columnExpr)?
    | nestedIdentifier columnTypeExpr? tableColumnPropertyExpr columnCommentClause? codecExpr? (TTL columnExpr)?
    ;
